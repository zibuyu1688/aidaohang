...
const existingUrls = new Set(websitesDatabase.flat().map(site => site.url));
console.log('当前数据库网站数量:', websitesDatabase.flat().length);
...
14、素材库：https://mp.weixin.qq.com/s/hx6jAtlADlZvgodaJY4R_A
15、优美图：http://www.youmeitu.com/
16、视觉 ME：http://shijue.me/
17、堆糖网：http://www.duitang.com/
...
12、Liveapp 场景应用：http://www.liveapp.cn/
...
6、Datahoop 大数据分析平台：http://www.datahoop.cn/
7、前瞻网：http://www.qianzhan.com/qzdata/list/147.html
8、数据观：http://www.cbdio.com/
9、易观：http://www.analysys.cn/
10、中国统计网：http://www.itongji.cn/cms/article/index
11、网站数据分析：http://webdataanalysis.net/
12、阿里指数：http://index.1688.com/
13、199IT：http://www.199it.com/
14、网站分析在中国：http://www.chinawebanalytics.cn/
15、DCCI 互联网数据中心：http://www.dcci.com.cn/
16、36大数据：http://www.36dsj.com/

表情包制作：
1、暴走漫画制作器：http://baozoumanhua.com/makers
2、豆饼软件网：http://www.doubean.com/
3、爱斗图：http://www.adoutu.com/pages/index.php
...
function parseData(data) {
    ...
    for (let line of lines) {
        ...
        if (match) {
            ...
            let url = match[2].replace(/\/$/, ''); // 移除末尾斜杠
            
            // 清理URL
            url = url.replace(/（[^）]*）$/, ''); // 移除末尾括号内容
            url = url.replace(/\?.*$/, ''); // 移除查询参数
            url = url.replace(/#.*$/, ''); // 移除锚点
            
            // 清理名称
            name = name.replace(/（.*?）/g, '').trim();
            name = name.replace(/\([^)]*\)/g, '').trim();
            
            const category = categoryMap[currentSection] || '工具';
...
console.log('解析到的新条目数量:', newEntries.length);

// 去重：过滤掉已存在的URL
const uniqueEntries = newEntries.filter(entry => {
    const normalizedUrl = entry.url.toLowerCase();
    return !existingUrls.has(entry.url) && 
           !Array.from(existingUrls).some(url => url.toLowerCase() === normalizedUrl);
});

console.log('去重后的新条目数量:', uniqueEntries.length);
console.log('重复条目数量:', newEntries.length - uniqueEntries.length);

if (uniqueEntries.length === 0) {
    console.log('没有新的条目需要添加');
    process.exit(0);
}

// 生成插入的JSON格式
const formatted = uniqueEntries.map(entry => `    {
        "name": "${entry.name}",
...
// 插入新条目
const newContent = content.slice(0, insertPoint) + ',\n' + formatted + content.slice(insertPoint);
fs.writeFileSync(websitesPath, newContent, 'utf8');

console.log('已添加新条目到 websites.js');

// 验证语法
try {
    delete require.cache[require.resolve(websitesPath)];
    const { websitesDatabase: newDb } = require(websitesPath);
    console.log('✓ 语法验证通过');
    console.log('新的数据库网站总数:', newDb.flat().length);
    console.log('实际新增数量:', newDb.flat().length - websitesDatabase.flat().length);
} catch (e) {
...
console.log('\n前10个新增条目:');
uniqueEntries.slice(0, 10).forEach((entry, i) => {
    console.log(`${i + 1}. ${entry.name} - ${entry.category} - ${entry.url}`);
});
